...
# ColorFormatter and setup_logging defined inline to avoid circular imports
...
# -------------------- Auth / Service --------------------
from dfr.auth import get_service_and_creds, get_service_if_token_valid, try_get_account_info, get_account_info
...
# -------------------- Helpers --------------------
from dfr.utils import extract_folder_id, safe_filename, ensure_dir, backoff_sleep, classify_media
from dfr.listing import gapi_execute_with_retry, get_item
...
# -------------------- Listing --------------------
from dfr.listing import resolve_folder, wait_if_paused, list_folder_recursive

def resolve_folder(service, url: str):
    folder_id = extract_folder_id(url)
    if not folder_id:
        logging.error(L(f"Folder URL had no ID: '{url}'", f"URL folder tidak memiliki ID: '{url}'"))
        # Add stack trace to help debug call flow for this logical error
        import traceback
...
def prescan_tasks() -> List[Dict]:
    ...
    # Pre-authenticate ONCE before parallel execution to prevent multiple OAuth flows
    shared_service, shared_creds = get_service_and_creds(TOKEN_FILE, CREDENTIALS_FILE)

    def _scan_one(url: str, service):
        ...
        if INTERRUPTED:
            return local_tasks
        try:
            # Use shared service (no need to re-authenticate)
            svc = service
            folder_id = extract_folder_id(url)
            name, ok = resolve_folder(svc, url)
            if not ok:
                return local_tasks
            ...
            logging.exception(L(f"Listing failed for URL {url}: {e}", f"Listing gagal untuk URL {url}: {e}"))
            return local_tasks
    ...
    with ThreadPoolExecutor(max_workers=max(1, int(CONCURRENCY) if str(CONCURRENCY).isdigit() else 1)) as ex:
        for url in urls:
            if INTERRUPTED: break
            futures.append(ex.submit(_scan_one, url, shared_service))
        for fut in as_completed(futures):
            try:
                ...
                logging.exception(L(f"Prescan worker error: {e}", f"Kesalahan prescan: {e}"))
...
from dfr.downloads import _download_video_simple
